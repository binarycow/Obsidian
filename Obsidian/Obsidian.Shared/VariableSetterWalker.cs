...
namespace Obsidian
{
    internal class VariableSetterWalker : ExpressionVisitor
    {
        ...
        protected override Expression VisitParameter(ParameterExpression node)
        {
            if(node.Name == null)
            {
                if (NullVariables.Contains(node) == false)
                {
                    NullVariables.Add(node);
                }
            }
            else
            {
                if (Variables.TryGetValue(node.Name, out var varHashSet) == false)
                {
                    varHashSet = new HashSet<ParameterExpression>();
                    Variables.Add(node.Name, varHashSet);
                }
                if (varHashSet.Contains(node) == false)
                {
                    varHashSet.Add(node);
                }
            }
